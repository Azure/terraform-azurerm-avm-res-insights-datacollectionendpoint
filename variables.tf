...
A map of diagnostic settings to create on the data collection endpoint. The map key is deliberately arbitrary to avoid issues where map keys maybe unknown at plan time.
...
variable "public_network_access" {
  ...
  validation {
    condition     = contains(["Disabled", "Enabled", "SecuredByPerimeter"], var.public_network_access)
    error_message = "The public_network_access must be one of: 'Disabled', 'Enabled', 'SecuredByPerimeter'."
  }
}
...
variable "sku" {
  ...
  default     = null
  description = <<DESCRIPTION
The SKU of the data collection endpoint resource.
